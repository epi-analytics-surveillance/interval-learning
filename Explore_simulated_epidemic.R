...
# Function to group, summarise, and plot data, by the infector infected time
...
# Function to group, summarise, and plot data, by the infectee infected time
summary_stats_back <- function(time_interval = 10){
  # Input: time-interval: Group data into time_intervals bins 
  # Initial value is timer_interval = 10, meaning days 1–10 = Group 1, 11–20 = Group 2, ..., 71–80 = Group 8
  
  # Group data by input interval
  data$time_group <- cut(data$date_infectee_infected, 
                         breaks = seq(0, 80, by = time_interval),
                         right = FALSE, 
                         include.lowest = TRUE)
