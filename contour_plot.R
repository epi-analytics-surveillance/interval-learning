...
ll_plot_data <- function(N = 100000, case_count_days = 50, I = case_count){
  ...
  data <- tibble(
    ...
    GI_sd = runif(N, min = 0.5, max = 1.5), # GI sd range from 1-3 days
    Rt_mean = runif(N, min = 0.5, max = 5), # Rt mean range from 0.5-5
...
data <- ll_plot_data(N = 100000, case_count_days = 50, I = case_count)
...
ggplot(reg_df, aes(x = GI_mean, y = Rt_mean, z = log_like)) +
  ...
  geom_point(aes(x = 4.5, y = 2.75), size = 5, shape = 10) +
  theme_minimal()
...
filled.contour(x = reg_data$x,
               ...
               key.title = title(main = "Log-Likelihood", cex.main = 1))
